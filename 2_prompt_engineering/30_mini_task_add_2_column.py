...
async def run_experiment(prompt_variant_name, system_prompt, user_prompt_template, test_case, temperature, model_name = "gpt-4.1-nano"):
    ...
    try:
        ...
        result.setdefault("human_score_relevance", "")
        result.setdefault("human_score_clarity", "")
        csv_file(result)
...
def csv_file(result):
    output_csv_file = "2_prompt_engineering/29_storing_experiment_data/experiment_results.csv" 
    ...
    fieldnames = [ 
        ...
        "system_prompt", "user_prompt", # Added these for completeness 
        "human_score_relevance", "human_score_clarity" # Placeholders 
    ]

    with open( output_csv_file, 'a', newline= '', encoding='utf-8') as csvfile: 
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames, extrasaction='ignore') # extrasaction='ignore' will ignore keys in dict not in fieldnames
        if not file_exists:
            writer.writeheader()
        writer.writerow(result)

    console.print(Markdown(f"Experiment results saved to {output_csv_file}"))
...
async def main_batch_runner():
    ...
    output_file = "2_prompt_engineering/25_cli_batch_test_data_result/experiment_result.json"
